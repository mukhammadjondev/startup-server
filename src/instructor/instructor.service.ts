...
export class InstructorService {
  ...
  async applyAsInstructor(dto: InstructorApplyDto) {
    ...
    if(user) {
      await this.userModel.findByIdAndUpdate(user._id, {$set: {job, fullName: `${firstName} ${lastName}`}})
    }
  ...
  async getInstructors(language: string, limit: string) {
    const instructors = await this.instructorModel.find({language, approved: true})
      .populate('author')
      .limit(Number(limit))
      .sort({createdAt: -1})
      .exec()
